...
uint8_t sampleLength[PDX_NUM_SAMPLES];
int samplesToWait = 0;
int samplesInLoop = 0, bytesInLoop = 0, bytesInVgm = 0;

char startSearchingLoop = 0;
...
void MXDRVG_Tick() {
	//VGM WRITING
	int samples = 441 * 1024 * (256 - G.MUSICTIMER) / 40000;
	samplesToWait += samples;
	OPMINTFUNC();
}
...
/***************************************************************/
static void PCM8_NOTE_ON( int sample ){
	if(sample&0x10){
		sample&=0x0F;
		sample*=2;
	}
	if(!startSearchingLoop){
		if(samplesToWait>0){
		...
		if(samplesToWait>0){
			bytesInLoop += w.writeWait(samplesToWait, 0);
...
*/
	...
	*(A4-D0-1) -= 1;
	...
	if ( *(A4) != 0xf1 ) goto L001396;			// Not END?, then continue
	if ( !(*(A4+1)) ) { L0013e6(); return; };	// END and with LOOP! GOTO L0013e6
